...
<title>Gestrione Ferrature</title>
</head>
<body>

	<%@ include file="../header.jsp"%>
	<div class="block-container">
		<h1>Cavallo: ${ferraturaAttr.cavallo.nome}</h1>
	</div>
	<div class="card-container">

		<form class="form-horizontal"
			action="${pageContext.request.contextPath }/eseCompletaFerraturaSer"
			method="post">
			
			<div class="card text-white bg-success"
				style="position: relative; width: 100%">
				<div class="card-header txt-center">
					<h3>Completa Ferratura</h3>
				</div>
				<div class="card-body center-card">


					<input type="hidden" value="${ferraturaAttr.cavallo.id}" name="idCavallo">
					<input type="hidden" value="${ferraturaAttr.cavallo.nome}" name="nomeCavallo">
					<input type="hidden" value="${ferraturaAttr.id}" name="idFerratura">
					<input type="hidden" value="${forAllList}" name="forAllList">
				...
				<div class="card-footer txt-center">
					<a href="javascript:history.back()" class="btn btn-primary btn-md">Torna
						Indietro</a>
					<button type="submit" id="submitId" class="btn btn-primary btn-md">Registra</button>
				</div>
			</div>
		</form>
	</div>
